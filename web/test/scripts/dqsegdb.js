	...
	$.get("scripts/actions.php?action=get_latest_query_results", function(r) {
		...
		$.get("scripts/actions.php?action=get_latest_json_payload_filename", function(fn) {
			...
			$.get("scripts/actions.php?action=get_latest_additional_payload_filename", function(afn) {
				if(afn != 0) {
					console.log(afn);
					$('#div_view_segments').append('<p class="w3-margin-0 w3-margin-top w3-margin-right w3-margin-left><strong>Additional payload:</strong></p><div id="div_add_payload" class="w3-container w3-border"><i class="fas fa-spinner w3-spin"></i> Getting additional payload...</div>');
