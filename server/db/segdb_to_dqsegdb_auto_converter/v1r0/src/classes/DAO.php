...
//			$log->write_to_log("Finished updating ".$t_c." segment totals for version: ".$v_id." (".$tot.")...");
	...
	private function convert_segments($s) {
		...
		$log = new Logger();
		// Log.
		$log->write_to_log("Converting segment".$t_c."s...");
		// Create PDO object
		$this->db_connect();
		// Get.
		if($res = $this->pdo->query("INSERT INTO tbl_segment".$t_b."
			...
			// Log.
			$log->write_to_log("Finished converting segment".$t_c."s");
		}
		// Otherwise, output error.
		else {
			// Log.
			$log->write_to_log("ERROR - Problem converting segment".$t_c."s - ".print_r($this->pdo->errorInfo()));
			// Set result to error to not automatically go on with segment summary insertion.
			$r = FALSE;
		}
		// Return.
